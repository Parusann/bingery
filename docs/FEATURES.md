...
- **Similar strip is currently dead**: the client requests `GET /api/anime/<id>/similar`, but no such backend route exists — the real endpoint is `GET /api/recommend/similar/<int:anime_id>` (`routes/recommend.py`). The request falls through to the SPA catch-all in `app.py` (returns `index.html`, 200), JSON parsing fails, the query errors, and `SimilarStrip` never renders.
